      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <div className="lg:col-span-7 bg-(--settings-card-bg) border border-(--sidebar-border) rounded-2xl p-5 shadow-xs space-y-5">
          ...
          <div className="space-y-3 pb-4 border-b border-(--sidebar-border)">
            ...
            {targetWallet && (
              <div className="flex items-center gap-2 text-[11px] font-medium text-(--text-color)/80 bg-(--bg-light) border border-(--sidebar-border) rounded-lg px-3 py-1.5 w-fit">
                <IconWallet size={14} className="text-(--primary-color)" />
                <span>
                  Cartera:{" "}
                  <strong className="text-(--headings-color)">{targetWallet.name}</strong>
                </span>
                <span className="text-(--text-color)/40">•</span>
                <span>
                  Moneda:{" "}
                  <strong className="text-(--headings-color)">{targetWallet.currency}</strong>
                </span>
              </div>
            )}
